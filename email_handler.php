...
function handleEmailSending($clientId) {
    ...
    // Handle Multiple File Uploads and Track Names
    $attachmentPaths = [];
    $attachmentNames = [];
    ...
    <?php if (!empty($attachmentNames)): ?>
        <h4>Annexures</h4>
        <ul>
            <?php foreach ($attachmentNames as $fileName): ?>
                <li>
                    <?php echo htmlspecialchars($fileName); ?>
                    (uploaded on: <?php echo date('d/m/Y'); ?>)
                </li>
            <?php endforeach; ?>
        </ul>
    <?php endif; ?>
    ...
    try {
        ...
        $mail->Username   = 'rajeshbairisetti234@gmail.com';
        $mail->Password   = 'ruew bbri fuac djsc';
