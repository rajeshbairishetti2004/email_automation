...
<body>
    ...
    <script>
        ...
        // Freeze current allocation (never change this)
        const fixedCurrentLabels = [...initialLabels];
        const fixedCurrentData = [...initialCurrentData];
        const fixedCurrentColors = [...initialColors];
        ...
        function updateCurrentChart() {
            ...
            currentChartInstance = new Chart(ctx, {
                type: 'pie',
                data: {
                    labels: fixedCurrentLabels,
                    datasets: [{
                        data: fixedCurrentData,
                        backgroundColor: fixedCurrentColors,
                        borderWidth: 1
                    }]
                },
                options: {
        ...
        function createRecommendedChart() {
        ...
        function refreshRecommendedChart() {
        ...
        submitAssetBtn.onclick = async () => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    rowIds.push(Number(result.id));
                    // Current is always 0 for new assets
    ...
    if (e.target.classList.contains('rec-input') && editMode) {
        ...
        const index = rowIds.findIndex(r => Number(r) === id);
        ...
        function updateLegends() {
            ...
            fixedCurrentLabels.forEach((asset, i) => {

                const row = document.createElement('div');
                row.className = 'legend-row';
                row.innerHTML = `
                    <span class="color-box" style="background:${fixedCurrentColors[i]}
"></span>
                    <span class="asset-name-display">${asset}</span> – ${fixedCurrentData[i].toFixed(1)}%
