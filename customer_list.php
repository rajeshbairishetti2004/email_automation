...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'update_company') {
        ...
        if (empty($pan) || empty($name)) {
    echo json_encode(['success' => false, 'error' => 'Invalid customer']);
    exit;
}

        try {
            $stmt = $pdo->prepare("UPDATE customer_list SET company = ? WHERE pan = ? AND name = ?");
            $stmt->execute([$company, $pan, $name]);
...
<body>
        ...
        <div class="table-wrapper">
            <table class="customer-table" id="customerTable">
                ...
                <tbody>
                    ...
                    ?>
                        <tr class="<?= $statusClass ?>" data-pan="<?= e($c['pan'], '') ?>" data-aum="<?= (float)$c['aum'] ?>">
                            <td>
                                <a class="client-link"
                    ...
                    &state=all"
                            ...
                            <td><?= e($c['email']) ?></td>
                            <td><?= e($c['mobile']) ?></td>
                            <td><?= e($c['family_head']) ?></td>
                            <td><?= e($c['city']) ?></td>
                            <td>
                                <span class="editable-company"
                                    onclick="editCompany(this, '<?= e($c['pan'], '') ?>', '<?= e($c['name'], '') ?>')"
                                    data-original="<?= e($c['company'], '') ?>">
                                    <?= e($c['company']) ?>
                                </span>
    ...
    <div id="deleteAllModal"
        ...
        <script>
            ...
            function editCompany(span, pan, name) {
                ...
                input.addEventListener('input', () => {
                    ...
                    autosaveTimer = setTimeout(() => {
                        saveCompany(input, pan, name, span, savingIndicator);
                    }, 1000);
                });
            ...
            function saveCompany(input, pan, name, span, indicator) {
